...
class Es extends Controller
{
    ...
    public function createAllGoodsDocs()
    {
        try{
            ...
            while(true){
                //查询商品数据 每次处理1000条
                $goods = \app\common\model\Goods::with('category_bind')->field('id,goods_name,goods_desc, goods_price,goods_logo,cate_id')->limit($i, 1000)->select();
                if(empty($goods)){
                    //查询结果为空，则停止
                    break;
                }
                ...
                foreach($goods as $v){
                    unset($v['cate_id']);
                    $es->add_doc($v['id'],$v->toArray(), 'goods_index', 'goods_type');
                }
                $i += 1000;
            }
            die('success');
